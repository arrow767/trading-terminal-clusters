//! OKX V5 public `trades` WS session loop.
//!
//! Структурно идентичен `bybit_session`:
//! 1. **Batched subscribe** — дробим на фреймы (`subscribe_payloads_batched`).
//! 2. **Client-initiated ping** — OKX рвёт idle через 30с; шлём текст `ping`
//!    каждые ~20с.
//! 3. **Batched trades** — `trades` канал шлёт `data: [trade, ...]`.
//! 4. **Topic-routing** — symbol из `arg.instId` (нормализован в канон),
//!    не из элемента data.
...
use exchange_okx::{OkxTradeParser, OkxWs};
...
pub async fn run_session(
    ...
    for payload in connector.subscribe_payloads_batched(&spec_refs) {
        sink.send(Message::Text(payload))
            .await
            .context("okx: send subscribe batch")?;
    }
    tracing::info!(symbols = routes.len(), "okx session subscribed");

    // Routing keyed by canonical symbol (spec.symbol уже канон BTCUSDT,
    ...
    let ping_text = match connector.ping_payload() {
        ...
        _ => "ping", // OKX всегда Text("ping"); защитимся.
    };
        ...
        tokio::select! {
            ...
            _ = ping_ticker.tick() => {
                if let Err(e) = sink.send(Message::Text(ping_text.to_string())).await {
                    return Err(anyhow::Error::from(e).context("okx: send ping"));
                }
                stats.pongs_sent += 1;
            }
            next = stream.next() => {
                match next {
                    ...
                    Some(Ok(Message::Text(t))) => {
                        // OKX отвечает на наш текст `ping` плоским `pong` —
                        // не JSON, пропускаем без попытки декода.
                    ...
                    Some(Ok(Message::Close(frame))) => {
                        tracing::info!(?frame, "okx ws close received");
                        break;
                    }
                    Some(Err(e)) if is_soft_close(&e) => {
                        tracing::info!(error = %e, "okx ws session ended (peer hung up)");
                        break;
                    }
                    Some(Err(e)) => {
                        return Err(anyhow::Error::from(e).context("okx ws read"));
                    }
                }
            }
        }
...
fn handle_payload(
    ...
    let v: serde_json::Value = match serde_json::from_slice(raw) {
        Ok(v) => v,
        Err(e) => {
            tracing::debug!(error = %e, "ignoring undecodable okx frame");
            stats.frames_undecodable += 1;
            return;
        }
    };
    let Some(symbol) = parser.peek_symbol(&v) else {
        // subscribe/error/event — не trade.
        return;
    };
    ...
    let trades = match parser.parse_value(&v, &route.spec) {
        Ok(ts) => ts,
        Err(e) => {
            tracing::warn!(error = %e, symbol = %symbol, "okx parse_value failed");
